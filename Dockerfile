...
RUN groupadd -g 61000 patsy \
    && useradd -g 61000 -l -r -u 61000 patsy
...
CMD ["sh", "-c", "alembic upgrade head && uvicorn patsy.main:app --host 0.0.0.0 --port 8000 --reload"]
...
USER patsy
CMD ["sh", "-c", "alembic upgrade head && uvicorn patsy.main:app --host 0.0.0.0 --port 8000 --proxy-headers"]
